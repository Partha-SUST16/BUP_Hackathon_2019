...
public class MemberDashboard extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference()
                .child("Member").child(FirebaseAuth.getInstance().getCurrentUser().getUid().toString())
                .child("vaccine");
        ...
        FirebaseDatabase.getInstance().getReference()
                .child("Member").child(FirebaseAuth.getInstance().getCurrentUser().getUid().toString()).child("name").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                name[0] = dataSnapshot.getValue(String.class);
                //Toast.makeText(getApplicationContext(),name[0]+" "+Integer.toString(doctorList.size()),Toast.LENGTH_SHORT).show();
            }
        ...
        gridview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                ...
                ((HwAdapter) parent.getAdapter()).getPositionList(selectedGridDate, MemberDashboard.this);
            }
